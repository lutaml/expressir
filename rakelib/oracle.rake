...
namespace :oracle do
  TAG = "eeng-5.2.8"
  BASE = "https://github.com/expresslang/eengine-releases/releases/download/#{TAG}"
  ...
  desc "Fetch the pinned eengine oracle (#{TAG}) into tmp/oracle/eengine " \
       "for the SHTOLO corpus differential"
  task :fetch do
    platform = [Gem::Platform.local.os, Gem::Platform.local.cpu]
    entry = ASSETS[platform]
    abort "no #{TAG} asset for #{platform.join('-')}" unless entry
    ...
    sh "curl", "-fSL", "-o", download, "#{BASE}/#{entry[:asset]}"

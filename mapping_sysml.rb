...
datatype_hash["BOOLEAN"] = 'http://www.omg.org/spec/SysML/20100301/SysML-profile.uml#Boolean'
datatype_hash["INTEGER"] = 'http://www.omg.org/spec/SysML/20100301/SysML-profile.uml#Integer'
datatype_hash["NUMBER"] = 'http://www.omg.org/spec/SysML/20100301/SysML-profile.uml#Number'
datatype_hash["REAL"] = 'http://www.omg.org/spec/SysML/20100301/SysML-profile.uml#Real'
datatype_hash["STRING"] = 'http://www.omg.org/spec/SysML/20100301/SysML-profile.uml#String'
...
valuetype_template = %{<sysml:ValueType base_DataType = "<%= baseType %>" xmi:id = "<%= xmiid %>"/>}
...
# Evaluate and write TYPE Select end template 
			...
			puts "UNHANDLED TYPE: " + select.name
		end
	end
...
for schema in schema_list
	...
	for entity in entity_list
		...
		baseClass = '_' + schema.name + '-' + entity.name
	...
	for type in type_list  
		...
		baseType = '_' + schema.name + '-' + type.name
	...
	for select in select_list
		if selectTypeType[select.name] == "ValueType"
			# Evaluate and write TYPE ValueType template
			baseType = '_' + schema.name + '-' + select.name
			xmiid = baseType + '-ValueType'
			res = ERB.new(valuetype_template)
		else
			# Evaluate and write ENTITY Block template 

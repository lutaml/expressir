...
==== Backend: Parslet vs Parsanol

Expressir supports two parser backends:

**Parslet (Pure Ruby)**::
* Original Ruby PEG parser
...
[source,ruby]
----
# Automatically uses Parsanol if available, falls back to Parslet
repo = Expressir::Express::Parser.from_file("geometry.exp")

# Force use of native parser (requires Parsanol)

...
RSpec.describe Expressir::Express::PrettyFormatter do
  describe "Integration tests with real schemas" do
    describe "simple_schema.exp" do
      let(:schema_path) do
        File.join(__dir__, "../../../examples/ler/simple_schema.exp")
      end
      ...
      it "can be re-parsed after formatting" do
        repository = Expressir::Express::Parser.from_file(schema_path)
        formatted = formatter.format(repository)
    ...
    describe "game_management_schema.exp" do
      let(:schema_path) do
        File.join(__dir__, "../../fixtures/examples/game_management_schema.exp")
      end
      let(:formatter) { described_class.new }
    ...
    describe "Edge cases" do
      ...
      it "handles schema with constants" do
        # Use geometry_schema which has a CONSTANT block
        schema_path = File.join(__dir__,
                                "../../fixtures/examples/geometry_schema.exp")
        repository = Expressir::Express::Parser.from_file(schema_path)
        formatted = formatter.format(repository)

        expect(formatted).to include("CONSTANT")
        expect(formatted).to include("END_CONSTANT")
      end
    ...
    describe "CONSTANT alignment" do
      ...
      it "aligns constants in schema-level CONSTANT block" do
        ...
        begin
          ...
          expect(colon_positions.uniq.length).to eq(1),
                                                 "Colons should be aligned"

          # Verify all assignment operators are at the same column
          assign_positions = lines.map { |line| line.index(":=") }
          expect(assign_positions.uniq.length).to eq(1),
                                                  "Assignment operators should be aligned"
        ensure
          temp_file.unlink
        end
      end
    ...
    describe "Backward compatibility" do
      ...
      it "both formatters produce parseable output" do
        repository = Expressir::Express::Parser.from_file(schema_path)
    ...
    describe "Round-trip formatting" do
      ...
      it "maintains semantic equivalence after multiple format cycles" do
        repository = Expressir::Express::Parser.from_file(schema_path)

...
# Create arm.xml
${EENG} --xml -mode arm_shortform -stepmod "${STEPMOD}/" -schema "${MODULE}/${line}/arm.exp" -stepmod_vcs off -out-dir "${MODULE}/${line}/" |& tee output-arm.sbcl
# Create mim.xml
${EENG} --xml -mode mim_shortform -stepmod "${STEPMOD}/" -schema "${MODULE}/${line}/mim.exp" -stepmod_vcs off -out-dir "${MODULE}/${line}/" |& tee output-mim.sbcl
...
${EENG} --compare -mode arm_shortform -stepmod "${STEPMOD}/" -reference_schema "${MODULE}/${line}"/arm.exp \
    -trial_schema "${MODULE}/${line}/arm-xml.exp" --xml-output -out-dir ./ |& tee "arm-compare.${LOG}"

# Convert mim.xml -> mim.htm
java -Xmx1025M -jar "${SAXON}"/saxon.jar -a -o "${MODULE}"/"${line}"/mim.htm "${MODULE}/${line}"/mim.xml \
    output_type="HTM" output_rcs="NO"
...
${EENG} --compare -mode mim_shortform -stepmod "${STEPMOD}/" -reference_schema "${MODULE}/${line}/mim.exp" -trial_schema "${MODULE}/${line}/mim-xml.exp" --xml-output -out-dir ./ |& tee "mim-compare.${LOG}"

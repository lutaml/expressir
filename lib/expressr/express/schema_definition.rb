require "expressr/express/entity"
require "expressr/express/type_parser"
require "expressr/express/global_rule"
  ...
  module Express
    class SchemaDefinition < ModelElement
      ...
      def find_namedtype_by_name(typename)
        ...
        Expressr.ui.info("*** NOT HERE " + typename + " IN " + self.name)
        Expressr.ui.info("*** SEARCHED " + all_schema_array.size.to_s + " SCHEMAS:")

        all_schema_array.each do |schema|
          Expressr.ui.info("  - " + schema.name)
        end
